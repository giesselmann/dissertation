...
The identification of third generation sequencing related publications is implemented in two steps of seed and extend.
First, publications with keywords in either title or abstract are marked as seed paper.
The second step extends clusters around seed paper by following their citation edges in the literature graph.
...
Of the 8.7k third generation sequencing cluster nodes, only 7.6k have citing or cited edges and form a sub-graph suitable for embedding.
To further link isolated connected components and for additional preservation of the global graph structure, we complement the third generation sub-graph by random walks in the \textbf{core\_20} backbone graph.
Each walk starts at a cluster node which is also part of the \textbf{core\_20} graph and follows citation edges within the \textbf{core\_20} backbone until another core-cluster intersection node is found or a maximum length is reached.
Edges of random walks are only used for embedding, the additional nodes are ignored in the subsequent content analysis.
Applying the same \textit{DeepWalk} configuration and dimensionality reduction as above results in the long-read graph embedding shown in Fig. \ref{fig:state_of_art:umap_lr}.

\begin{figure}[h]
	\centering
	\includegraphics[width=1.0\textwidth]{figures/state_of_art/umap_lr.pdf}
	\captionsetup{format=plain}
	\caption[Third generation sequencing cluster]{Third generation sequencing cluster: The UMAP projection shows the \textit{DeepWalk} graph embedding with points scaled by number of citations. Colors indicated technology and seed/extend source.}
...
Aside from small outlier groups, a broad PacBio field is accompanied by two distinct accumulations of nanopore related work.

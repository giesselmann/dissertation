...
Beyond that, the token 'assembl' is one of the most prominent, and with different scaling found in almost all clusters.
...
\begin{figure}[h]
	...
	\captionsetup{format=plain}
	\caption[Third generation sequencing applications]{Third generation sequencing applications: The UMAP projection shows the \textit{DeepWalk} graph embedding with points scaled by number of citations. Colors indicate cluster with labels derived of most descriptive word tokens.}
...
The content of the composite 'MinION' cluster (ID 15) is further analyzed in section \ref{sec:state_of_art:nanopore}, concluding with the investigation of use cases within the so far hidden epigenetics.
